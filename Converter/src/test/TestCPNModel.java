...
public class TestCPNModel {
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length < 3) {
			System.out
					.println("cpnConverter.jar [xpdl file] [output cpn file] [simulation data]");
			return;
		}
		 ...
		 * 
		 * 
		 * Properties configFile = new Properties(); configFile.load(new
		 * FileInputStream("configuration.conf"));
		 * 
		 * String template = configFile.getProperty("TEMPLATE_PATH"); String
		 * layouter = configFile.getProperty("LAYOUTER"); String useLayouter =
		 * configFile.getProperty("USE_LAYOUTER");
		 * if(useLayouter.equals("TRUE")) doLayout = true;
		 * 
		 * File xpdlFile = new File("./files/models/subprocess_error.xpdl");
		 * File simDataFile = new
		 * File("./files/models/subprocess_timer_error.xml");
		 * 
		 * Parser p = new Parser(template); p.setElementFactory(new
		 * BPMNRelayFactory(p)); p.setElementParser(new
		 * XPDL2ElementParser(xpdlFile)); p.setSimDataParser(new
		 * KBSimDataParser(simDataFile)); p.parse();
		 * 
		 * p.save("./out.cpn", doLayout,layouter);
		 */
	}
}
