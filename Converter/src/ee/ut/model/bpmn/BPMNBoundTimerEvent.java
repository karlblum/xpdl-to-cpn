...
public class BPMNBoundTimerEvent extends BPMNElement{
	...
	public BPMNBoundTimerEvent(CPNProcess cPNProcess, Object obj, ElementParser elementParser) {
		super(cPNProcess);
		...
		String transId = cPNProcess.getCpnet().addTrans(elementName + " EVENT").getId();
		outputPlaceId = cPNProcess.getCpnet().addPlace(elementName + " OUT").getId();
	...
	public void addSimulationData(SimDataParser simDataParser) {
		int timer = simDataParser.getBoundTimer(elementId);
		BPMNTask2 task = (BPMNTask2) cPNProcess.getElement(taskId);
