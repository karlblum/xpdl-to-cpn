...
public class BPMNeXORGateway implements BPMNeGateway {
	...
	public BPMNeXORGateway(Page page, String name, GWType type,
		...
		if (type == GWType.SPLIT || type == GWType.MIXED) {
			...
			Arc a1 = BPMNeUtil.createArc(page, transition, splittingPlace,
					(XmlString) variable.copy());
			splitArcs.add(a1);
		}
		...
		if (type == GWType.JOIN || type == GWType.MIXED) {
			...
			Arc a1 = BPMNeUtil.createArc(page, joiningPlace, transition,
					(XmlString) variable.copy());
			entryArc = a1;
		}
	}
	...
	public Place addEntryPlace(String from) {
		...
		if (type == GWType.SPLIT) {
			...
			BPMNeUtil.createArc(page, trans, joiningPlace, (XmlString) variable
					.copy());
		}
		...
		Arc arc = BPMNeUtil.createArc(page, entryPlace, trans,
				(XmlString) variable.copy());
	...
	public Place addExitPlace(String to) {
		...
		if (type == GWType.JOIN) {
			...
			trans = BPMNeUtil.createTrans(page, name + "\npath_to_" + to);
			Arc a1 = BPMNeUtil.createArc(page, splittingPlace, trans,
					(XmlString) variable.copy());
			splitArcs.add(a1);
		}
		...
		BPMNeUtil
				.createArc(page, trans, exitPlace, (XmlString) variable.copy());
	...
	public void setTransitionProbabilities(Object[] probs) {
		...
		for (int i = 0; i < probs.length; i += 2) {
			...
			if (path < (num - 1))
				expression += String.format(
						"if prob < %d then (caseInfop, %d)\n", current + prob,
						path);
			else
				expression += String.format("(caseInfop, %d)\nend", path);

			Trans trans = exitTrans.get(to);
			trans.getCondArray(0).getText().set(
					XmlString.Factory
							.newValue(String.format("[path=%d]", path)));
		...
		for (Arc a : splitArcs)
			a.getAnnot().getText().set(
					XmlString.Factory.newValue("(caseInfo, path)"));

		transition.getCodeArray(0).getText().set(
				XmlString.Factory.newValue(expression));
		splittingPlace.getTypeArray(0).getText().set(
				XmlString.Factory.newValue("CASExPATH"));
		entryArc.getAnnot().getText().set(
				XmlString.Factory.newValue("caseInfop"));
	}
}
