...
public final class Simulator extends Observable {
	...
	/**
	 * @param implementation implementation delegate
	 */
	 ...
	 * @param source source to be notified of updates
	 * @return the result of the evaluation
	 * @throws Exception if ML raises an exception
	 */
	 ...
	 * @throws Exception the exception raised by ML if any
	 */
	 ...
	 * @return reply
	 * @throws IOException if transmitting failed due to network error
	 */
	...
	Packet handle(final Packet p) {
		try {
			...
			if (p.getReturnType() != null && !p.getReturnType().equals(Void.class) && result != null
			        && p.getReturnType().isAssignableFrom(result.getClass())) { return new Packet(5, Collections
			        .singletonList(result)); }
		} catch (final Exception e) {
			// Ignore if we do not return normally
		}
		if (String.class.equals(p.getReturnType())) { return new Packet(5, Collections
		        .singletonList("Error in execution")); }
