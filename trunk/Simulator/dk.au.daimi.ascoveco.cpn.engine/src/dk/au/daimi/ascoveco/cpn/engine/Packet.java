...
public class Packet implements Serializable {
	...
	public Packet(final int opcode, final int command) {
		if (opcode != 7 && opcode != 9) { throw new IllegalArgumentException("Trying to create GPC with wrong opcode"); }
		this.opcode = opcode;
	...
	private byte[] getByteArray(final DataInputStream in) throws IOException {
	...
	private String getString(final DataInputStream in) throws IOException {
	...
	public void receive(final DataInputStream in) throws IOException {
		...
		if (isBIS()) {
			...
			final int bs = in.readInt();
			final int is = in.readInt();
			final int ss = in.readInt();
			...
			final int args = in.readInt() - 1; // Return is also counted
			if (Packet.debug) {
			...
			for (int j = 0; j < args; ++j) {
				if (in.readInt() != 1) { throw new IOException("Recieved parameter INOUT or OUT; neither is supported."); }
				switch (in.readInt()) {
				case 1:
					parameters.add(in.readInt());
					break;
				case 2:
					parameters.add(getString(in));
					break;
				case 3:
					parameters.add(in.readInt() != 0);
					break;
				case 4:
					break;
				case 5:
				...
				case 7:
					...
					break;
				case 8:
					parameters.add(getByteArray(in));
					break;
				default:
					assert false;
				}
