...
import dk.au.daimi.ascoveco.cpn.engine.highlevel.Util;
...
import dk.au.daimi.ascoveco.cpn.model.Object;
...
public class LocalChecker {
	...
	private void checkUniquePageNames(final PetriNet petriNet) throws LocalCheckFailed {
		...
		for (final Page page : petriNet.getPage()) {
			...
			if (names.containsKey(name)) { throw new LocalCheckFailed(page.getId(), "Page name `"
			        + page.getName().getText() + "' is not unique (it is translated to `" + name + "', which clashes)"); }
			names.put(name, page);
		}
	}
	...
	private void checkUniqueNames(final Page page) throws LocalCheckFailed {
		...
		for (final Object object : page.getObject()) {
			if (object instanceof PlaceNode || object instanceof Instance || object instanceof TransitionNode) {
				final Node node = (Node) object;
				final String name = getName(node);
				if (names.containsKey(name)) { throw new LocalCheckFailed(node.getId(), getNodeName(node)
				        + "' is not unique  page `" + getName(page) + "' (it is translated to `" + name
				        + "', which clashes with " + getNodeName(names.get(name)) + ")"); }
				names.put(name, node);
			}
		}
	}
	...
	private void checkAllNames(final Page page) throws LocalCheckFailed {
		if ("".equals(getName(page))) { throw new LocalCheckFailed(page.getId(),
		        "Page has no or illegal name (name is `" + page.getName().getText() + "')"); }
		for (final Object object : page.getObject()) {
			// System.out.println(object);
			if ("".equals(getName(object))) { throw new LocalCheckFailed(object.getId(), "Object on page `"
			        + getName(page) + "' has no or illegal name (name is `" + object.getName().getText() + "')"); }
		}
	}
}
